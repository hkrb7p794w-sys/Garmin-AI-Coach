"""Verwaltung des Chatverlaufs für die freie Gemini-Chat-Funktion im
Dashboard-Tab "Chat" (siehe claude/status-und-plan.md im Projekt für den
Auftrag von Alex, 11.09.2026: "Chat integrieren, sodass ich gezielte Fragen
über die API an Gemini stellen kann").
...
zurück), MIT dem aktuellen Trainingskontext (Readiness, VO2max, Phase,
...
zurückliegende Frage-Antwort-Paare bleiben sichtbar).
...
# werden. Begrenzt, damit die Datei nicht unbegrenzt wächst und die
# Markdown-Card im Dashboard nicht endlos lang wird (analog zu HISTORY_DAYS/
# STRENGTH_CACHE_DAYS in app.py).
MAX_STORED = 30
# Wie viele der letzten Austausche als Kontext in den nächsten Gemini-Prompt
# wandern (siehe context_for_prompt) - klein gehalten, damit der Prompt nicht
# unnötig gross wird; für echte Anschlussfragen reichen wenige reichen.
...
def add_exchange(question: str, answer: str) -> list:
    """Hängt eine neue Frage+Antwort an den Verlauf an, begrenzt auf
    MAX_STORED Einträge (älteste fallen zuerst raus). Gibt den kompletten
    (bereits begrenzten) Verlauf zurück."""
    entries = _load()
    entries.append({
        "question": question,
        "answer": answer,
...
def all_exchanges() -> list:
    """Kompletter gespeicherter Verlauf, chronologisch (älteste zuerst) -
    für die Anzeige im Dashboard (dort neueste zuerst sortiert per
    Jinja-`reverse`, siehe Dashboard-View "chat")."""
    return _load()


def recent(n: int = DEFAULT_CONTEXT_TURNS) -> list:
    """Die letzten n Austausche, chronologisch (älteste zuerst) - für den
    Konversations-Kontext im nächsten Gemini-Prompt (siehe
...
def context_for_prompt(n: int = DEFAULT_CONTEXT_TURNS) -> str:
    """Baut den Konversationsverlauf-Block für den nächsten Gemini-Prompt,
    damit Anschlussfragen ("und wie sieht das bei X aus?") funktionieren.
    Gibt "" zurück, wenn es noch keinen Verlauf gibt (dann taucht im Prompt
    auch kein leerer/nutzloser Abschnitt auf)."""
    entries = recent(n)
    if not entries:
        return ""
    lines = ["Bisheriger Gesprächsverlauf in diesem Chat (für Anschlussfragen):"]
